...
def get_clip_embeddings(vocabulary, clip_model):
    embs = clip_model.encode_text(clip.tokenize(vocabulary).cuda()).permute(1, 0).float().cpu()
    return (embs / torch.linalg.norm(embs, dim = 0))
...
class SAMLabelledDataset(Dataset):
    ...
    def __init__(
        ...
        self._setup_detic_all_classes(view_data, clip_model)
        self._setup_detic_dense_labels(
            dataset, images_to_label, clip_model, sentence_model, preprocess
        )
    ...
    def _setup_detic_dense_labels(
        self, dataset, images_to_label, clip_model, sentence_model, preprocess
    ):
        # Now, get all the sentence encoding for all the labels.
        text_strings = [
            SAMLabelledDataset.process_text(x) for x in self._all_classes
        ]
        with torch.no_grad():
            all_embedded_text = sentence_model.encode(text_strings)
            all_embedded_text = torch.from_numpy(all_embedded_text).float()
            clip_head = get_clip_embeddings(text_strings, clip_model)
        ...
        for idx, data_dict in tqdm.tqdm(
            enumerate(dataloader), total=len(dataset), desc="Calculating SAM features"
        ):
            ...
            for image, coordinates in zip(rgb, xyz):
                ...
                with torch.no_grad():
                    result = self._predictor.generate(image.permute(1, 2, 0).numpy())
                ...
                features = []
                result = sorted(
                    result, key=lambda x: x["area"], reverse=True
                )
                for mask in result:
                    #if mask['bbox'][2] > 30 and mask['bbox'][3] > 30 and mask['bbox'][2] < 600 and mask['bbox'][3] < 600:
                    if mask['bbox'][2] > 30 and mask['bbox'][3] > 30:
                        masks.append(mask)
                        crop_image = image[:, max(0, mask['bbox'][1] - mask['bbox'][3] // 3) :
                                           min(mask['bbox'][1] + mask['bbox'][3] + mask['bbox'][3] // 3, self._image_height), 
                                          max(0, mask['bbox'][0] - mask['bbox'][2] // 3) :
                                           min(mask['bbox'][0] + mask['bbox'][2] + mask['bbox'][2] // 3, self._image_width)]
                        #crop_image = image[:, mask['bbox'][1] : mask['bbox'][1] + mask['bbox'][3], 
                        #                   mask['bbox'][0] : mask['bbox'][0] + mask['bbox'][2]]
                        #seg_image = torch.clone(image)
                        #seg_image[:, ~mask['segmentation']] = 0
                        #crop_image = seg_image[:, mask['bbox'][1] : mask['bbox'][1] + mask['bbox'][3], 
                        #                      mask['bbox'][0] : mask['bbox'][0] + mask['bbox'][2]]
                        feature = clip_model.encode_image(
                              preprocess(transforms.ToPILImage()(crop_image)).cuda().unsqueeze(0)).cpu().float().squeeze(0)
                        del crop_image
                        features.append(feature)
                        label = torch.argmax(feature @ clip_head)
                        score = torch.max(feature @ clip_head) / torch.linalg.norm(feature)
                        labels.append(label)
                        image_captions.append(text_strings[label.item()] + " (" + '{0:.2f}'.format(score.item()) + ")")
                ...
                #    out = v.draw_instance_predictions(instance.to("cpu"))
                    ...
                    annotated_image = annotator.annotate(scene=image.numpy().copy(), detections = detections, labels = image_captions)
                    del image_captions
                    #annotated_image = annotator.annotate(scene=image.numpy().copy(), detections = detections)
                    #print(str(self._visualization_path / f"{idx}.jpg"))
                    cv2.imwrite(
                    ...
                    cv2.imwrite(
                        str(self._visualization_path / f"Clean_{idx}.jpg"),
                        image.numpy()[:, :, ::-1],
                        [int(cv2.IMWRITE_JPEG_QUALITY), 80],
                    )
        ...
        if self._use_lseg:
            ...
            for idx, data_dict in tqdm.tqdm(
                ...
                for image, coordinates in zip(rgb, xyz):
                    ...
                    # print(image.shape, coordinates.shape)
                    with torch.no_grad():
    ...
    def _setup_detic_all_classes(self, view_data: R3DSemanticDataset, clip_model):
        ...
        mask_generator = SamAutomaticMaskGenerator(sam, points_per_side = 24, pred_iou_thresh = 0.95)
        prebuilt_class_names = [
            SAMLabelledDataset.process_text(x)
        ...
        #mask_generator.clip_head = get_clip_embeddings(new_metadata.thing_classes, clip_model)
        # Reset visualization threshold
        self._predictor = mask_generator
